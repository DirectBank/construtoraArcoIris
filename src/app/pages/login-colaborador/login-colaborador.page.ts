...
import { AuthenticationColaboradorService } from 'src/app/services/authentication-colaborador/authentication-colaborador.service';
...
@Component({
  ...
  templateUrl: './login-colaborador.page.html',
  styleUrls: ['./login-colaborador.page.scss'],
})

export class LoginColaboradorPage implements OnInit {
  ...
  constructor(
    ...
    private authService: AuthenticationColaboradorService,
  ...
  async logInClick() {
    ...
    await this.authService.logInColaborador(this.login, this.salvarSenha, this.device)
      .then((response: any) => {
        ...
        if (this.env.empresaAtual.personalizado == 2) {
          ...
          if (response.contratos && response.contratos.length == 1) {
            this.router.navigate(["/home-colaborador"], params);
          }
          else {
            this.router.navigate(["/tabs"], params);
          }
